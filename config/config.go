...
type Config struct {
	...
	jwtExpiration        time.Duration `cmd:"jwt-expiration" env:"JWT_EXPIRATION" desc:"JWT access expiration time: 30s, 20m, 1h"`
	jwtRefreshExpiration time.Duration `cmd:"jwt-refresh-expiration" env:"JWT_REFRESH_EXPIRATION" desc:"JWT refresh expiration time: 30s, 20m, 1h"`
	...
	dbMigrate            bool          `cmd:"db-migrate" env:"DB_MIGRATE" desc:"Apply database migrations" default:"true"`
