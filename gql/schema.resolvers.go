...
func (r *queryResolver) Users(ctx context.Context, offset int32, limit int32, sort model.Sort) ([]*model.User, error) {
	ctx, span := tracing.InitSpan(ctx, "/query/Users")
	defer span.End()
	userId, role := auth.GetContextUser(ctx)
	if role == model.RoleAdmin {
		return r.userRepository.GetUsers(ctx, offset, limit, sort)
	} else {
...
func (r *queryResolver) Orders(ctx context.Context, offset int32, limit int32, sort model.Sort) ([]*model.Order, error) {
	ctx, span := tracing.InitSpan(ctx, "/query/Orders")
	defer span.End()
	userId, role := auth.GetContextUser(ctx)
	if role == model.RoleAdmin {
		return r.orderRepository.GetOrders(ctx, offset, limit, sort)
	} else {
		return r.orderRepository.GetUserOrders(ctx, userId, offset, limit, sort)
	}
}
...
func (r *queryResolver) Products(ctx context.Context, offset int32, limit int32, sort model.Sort) ([]*model.Product, error) {
	ctx, span := tracing.InitSpan(ctx, "/query/Products")
	defer span.End()
	return r.productRepository.GetProducts(ctx, offset, limit, sort)
}
...
func (r *queryResolver) Reviews(ctx context.Context, offset int32, limit int32, sort model.Sort) ([]*model.Review, error) {
	ctx, span := tracing.InitSpan(ctx, "/query/Reviews")
	defer span.End()
	return r.reviewRepository.GetReviews(ctx, offset, limit, sort)
}
